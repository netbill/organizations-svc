...
func (s Service) CreateCity(ctx context.Context, params city.CreateParams) error {
	err := s.sql(ctx).CreateCity(ctx, pgdbsql.CreateCityParams{
...
func (s Service) UpdateCityStatus(ctx context.Context, ID uuid.UUID, status string) error {
	return s.sql(ctx).UpdateCityStatus(ctx, pgdbsql.UpdateCityStatusParams{
		ID:     ID,
		Status: pgdbsql.CitiesStatus(status),
	})
}
...
func (s Service) FilterCities(
	...
	params := pgdbsql.FilterCitiesParams{
		AgglomerationID: nilx.UUID(filter.AgglomerationID),
		NameLike:        nilx.String(filter.Name),
	}

	if filter.Status != nil {
		params.Status = pgdbsql.NullCitiesStatus{
			Valid:        true,
			CitiesStatus: pgdbsql.CitiesStatus(*filter.Status),
		}
	}
	...
	count, err := s.sql(ctx).CountCities(ctx, pgdbsql.CountCitiesParams{
...
func (s Service) FilterCitiesNearest(
	...
	params := pgdbsql.FilterCitiesNearestParams{
		AgglomerationID: nilx.UUID(filter.AgglomerationID),
		NameLike:        nilx.String(filter.Name),
		UserLat:         point[1],
		UserLng:         point[0],
	...
	count, err := s.sql(ctx).CountCitiesNearest(ctx, pgdbsql.CountCitiesNearestParams{
		AgglomerationID: params.AgglomerationID,
		Status:          params.Status,
		NameLike:        params.NameLike,
	})
