...
func (c Consumer) ProfileUpdated(ctx context.Context, event kafka.Message) error {
	return c.inbox.Transaction(ctx, func(ctx context.Context) error {
		...
		if _, err = c.inbox.UpdateInboxEventStatus(ctx, eventInBox.ID, c.callbacks.ProfileUpdate(ctx, eventInBox)); err != nil {
			c.log.Errorf(
