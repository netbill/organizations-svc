...
func NewCityQ(db pgdb.DBTX) CityQ {
...
func (q CityQ) Insert(ctx context.Context, data City) (City, error) {
...
func (q CityQ) FilterByID(id uuid.UUID) CityQ {
...
func (q CityQ) FilterByAgglomerationID(id uuid.UUID) CityQ {
...
func (q CityQ) FilterByStatus(status string) CityQ {
...
func (q CityQ) FilterBySlug(slug string) CityQ {
...
func (q CityQ) FilterLikeName(name string) CityQ {
...
func (q CityQ) UpdateOne(ctx context.Context) (City, error) {
...
func (q CityQ) UpdateMany(ctx context.Context) (int64, error) {
	q.updater = q.updater.Set("updated_at", time.Now().UTC())
...
func (q CityQ) UpdateAgglomerationID(id *uuid.UUID) CityQ {
	q.updater = q.updater.Set("agglomeration_id", id)
	return q
}

func (q CityQ) UpdateStatus(status string) CityQ {
	q.updater = q.updater.Set("status", status)
	return q
}

func (q CityQ) UpdateSlug(slug *string) CityQ {
	q.updater = q.updater.Set("slug", slug)
	return q
}

func (q CityQ) UpdateName(name string) CityQ {
	q.updater = q.updater.Set("name", name)
	return q
}

func (q CityQ) UpdateIcon(icon *string) CityQ {
	q.updater = q.updater.Set("icon", icon)
	return q
}

func (q CityQ) UpdateBanner(banner *string) CityQ {
	q.updater = q.updater.Set("banner", banner)
	return q
}

func (q CityQ) UpdatePoint(point any) CityQ {
	q.updater = q.updater.Set("point", point)
	return q
}

func (q CityQ) OrderName(asc bool) CityQ {
	if asc {
...
func (q CityQ) CursorCreatedAt(limit uint, asc bool, createdAt time.Time, id uuid.UUID) CityQ {
	if asc {
		q.selector = q.selector.OrderBy("c.created_at ASC", "c.id ASC")
	} else {
		q.selector = q.selector.OrderBy("c.created_at DESC", "c.id DESC")
	}

	q.selector = q.selector.Limit(uint64(limit))

	if asc {
		q.selector = q.selector.Where(sq.Expr("(c.created_at, c.id) > (?, ?)", createdAt, id))
	} else {
		q.selector = q.selector.Where(sq.Expr("(c.created_at, c.id) < (?, ?)", createdAt, id))
	}
...
func (q CityQ) Count(ctx context.Context) (int64, error) {
