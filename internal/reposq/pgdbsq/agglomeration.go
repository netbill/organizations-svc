...
func NewAgglomerationQ(db pgdb.DBTX) AgglomerationQ {
	builder := sq.StatementBuilder.PlaceholderFormat(sq.Dollar)
	return AgglomerationQ{
		db:       db,
...
func (q AgglomerationQ) New() AgglomerationQ {
	return NewAgglomerationQ(q.db)
}

func (q AgglomerationQ) Insert(ctx context.Context, data Agglomeration) (Agglomeration, error) {
	...
	err = data.scan(q.db.QueryRowContext(ctx, query, args...))
	if err != nil {
...
func (q AgglomerationQ) FilterByID(id uuid.UUID) AgglomerationQ {
...
func (q AgglomerationQ) Get(ctx context.Context) (Agglomeration, error) {
...
func (q AgglomerationQ) Select(ctx context.Context) ([]Agglomeration, error) {
...
func (q AgglomerationQ) UpdateOne(ctx context.Context) (Agglomeration, error) {
...
func (q AgglomerationQ) UpdateMany(ctx context.Context) (int64, error) {
	q.updater = q.updater.Set("updated_at", time.Now().UTC())
...
func (q AgglomerationQ) UpdateName(name string) AgglomerationQ {
	q.updater = q.updater.Set("name", name)
	return q
}

func (q AgglomerationQ) UpdateIcon(icon string) AgglomerationQ {
	q.updater = q.updater.Set("icon", icon)
	return q
}

func (q AgglomerationQ) UpdateStatus(status string) AgglomerationQ {
	q.updater = q.updater.Set("status", status)
	return q
}

func (q AgglomerationQ) OrderName(asc bool) AgglomerationQ {
	if asc {
...
func (q AgglomerationQ) CursorName(
	limit uint,
	asc bool,
	name string,
	id uuid.UUID,
	...
	if asc {
		q.selector = q.selector.Where(sq.Expr("(name, id) > (?, ?)", name, id))
	} else {
		q.selector = q.selector.Where(sq.Expr("(name, id) < (?, ?)", name, id))
	}
...
func (q AgglomerationQ) Count(ctx context.Context) (int64, error) {
