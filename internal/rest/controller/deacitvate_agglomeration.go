...
func (s Service) DeactivateAgglomeration(w http.ResponseWriter, r *http.Request) {
	...
	res, err := s.domain.Agglomeration.DeactivateAgglomerationByUser(
		r.Context(),
		initiator.ID,
		agglomerationID,
	)
	if err != nil {
		s.log.WithError(err).Errorf("failed to deactivate agglomeration")
		switch {
